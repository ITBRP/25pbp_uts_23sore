...
// DATABASE
// ============================
$conn = new mysqli("localhost", "root", "", "buku_data");
...
// cek data ada?
$q = $conn->prepare("SELECT image FROM only WHERE id=?");
$q->bind_param("i", $id);
$q->execute();
$r = $q->get_result();
...
$u = $conn->prepare("UPDATE only SET name=?, category=?, price=?, stock=?, image=? WHERE id=?");
