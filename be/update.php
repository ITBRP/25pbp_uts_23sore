...
// DATABASE
// ============================
$conn = new mysqli("localhost", "root", "", "2355201060_dila");
...
// cek data ada?
$q = $conn->prepare("SELECT image FROM produk WHERE id=?");
$q->bind_param("i", $id);
$q->execute();
$r = $q->get_result();
...
$u = $conn->prepare("UPDATE produk SET name=?, category=?, price=?, stock=?, image=? WHERE id=?");
